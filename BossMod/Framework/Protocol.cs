...
namespace BossMod
{
    ...
    class Protocol
    {
        public enum Opcode
        {
            ActionEffect1 = 0x00FF,
            ActionEffect8 = 0x032B,
            ActionEffect16 = 0x02B1,
            ActionEffect24 = 0x00B4,
            ActionEffect32 = 0x0208,
            ActorCast = 0x017F,
            ActorControl = 0x02C3,
            ActorControlTarget = 0x03E2,
            ActorControlSelf = 0x0302,
            ActorGauge = 0x032E,
            PresetWaymark = 0x02C2,
            Waymark = 0x0138,

            EffectResult1 = 0x0174,
            EffectResult4 = 0x01BB,
            EffectResult8 = 0x0155,
            EffectResult16 = 0x02CE,

            EffectResultBasic1 = 0x03D0,
            ...
            EnvControl = 0x0378, // updated - size=16, look for a bunch of messages starting with 0x8003759F after P1N intemperance cast...

            // 48 89 5C 24 08 57 48 83 EC ? 48 8B DA 33 FF 90
            UpdateRecastTimes = 0x0301, // payload = 80 floats 'elapsed' + 80 floats 'total'

            // S: 41 89 0C 02 48 8D 40 08
            UpdateHate = 0x034F, // payload = byte length + 3 bytes padding + { uint objID, byte enmity, byte padding[3] }[len]

            // S: E8 ? ? ? ? 4C 8B C0 BA ? ? ? ? 48 63 C6
            UpdateHater = 0x0291, // payload = byte length + 3 bytes padding + { uint objID, byte enmity, byte padding[3] }[len]
