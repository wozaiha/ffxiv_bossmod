...
namespace BossMod
{
    ...
    class Protocol
    {
        public enum Opcode
        {
            ...
            // Second match
            EffectResult1 = 0x0378, // Size 0x60
            EffectResult4 = 0x0290, // Size 0x168
            EffectResult8 = 0x0340, // Size 0x2C8
            EffectResult16 = 0x0395, // Size 0x588

            // 48 83 EC ? 45 0F B7 D0 41 81 FA ? ? ? ? 77 ? 74 ? 41 81 EA ? ? ? ?
            EffectResultBasic1 = 0x0258, // Size 0x18
            EffectResultBasic4 = 0x036C, // Size 0x48
            EffectResultBasic8 = 0x02EA, // Size 0x88
            EffectResultBasic16 = 0x038D, // Size 0x108
            EffectResultBasic32 = 0x0131, // Size 0x208
            EffectResultBasic64 = 0x0222, // Size 0x408
            ...
            EnvControl = 0x030E, // updated - size=16, look for a bunch of messages starting with 0x8003759F after P1N intemperance cast...

            // 48 89 5C 24 08 57 48 83 EC ? 48 8B DA 33 FF 90
            UpdateRecastTimes = 0x0300, // payload = 80 floats 'elapsed' + 80 floats 'total'

            // S: 41 89 0C 02 48 8D 40 08
            UpdateHate = 0x0331, // payload = byte length + 3 bytes padding + { uint objID, byte enmity, byte padding[3] }[len]

            // S: E8 ? ? ? ? 4C 8B C0 BA ? ? ? ? 48 63 C6
            UpdateHater = 0x00BD, // payload = byte length + 3 bytes padding + { uint objID, byte enmity, byte padding[3] }[len]
